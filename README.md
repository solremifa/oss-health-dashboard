...
flowchart TB
    ...
    subgraph frontend["frontend/ — /에 마운트"]
        FE["app.js<br/>DOM · 손으로 쓴 SVG"]
        FM["format.js<br/>값 → 문자열 · 치수<br/>(DOM을 모르는 순수 함수)"]
        FM --> FE
    end
...
**대시보드에 차트 라이브러리를 쓰지 않았습니다.** 지표가 4개(비율 3 + 시간 1)뿐이라
CDN 의존성을 더할 이유가 없고, CSP·오프라인·재현성에서 손해입니다. 막대와 도넛은
SVG로 직접 그렸습니다. 도넛은 호(arc) 경로 대신 **둘레가 정확히 100이 되는 반지름**
(`2πr = 100`)의 원에 `stroke-dasharray`를 얹는 방식이라, 대시 길이가 곧 백분율이 되어
삼각함수가 필요 없습니다.

**값을 다루는 함수는 DOM에서 떼어냈습니다.** `format.js`는 DOM도 `fetch`도 모르는
순수 함수만 담고 있어 Node 내장 러너로 검증됩니다. 그래서 npm 의존성 없이도
"`null`을 0으로 바꾸지 않는가"를 테스트로 고정할 수 있습니다 — 지표 계산과 API가
지켜 온 규칙이 마지막으로 깨지기 쉬운 자리가 화면입니다.
...
| 린트·포맷 / 테스트 / CI | ruff / pytest · `node --test` / GitHub Actions |
...
프론트엔드는 Node 내장 러너로 검증합니다. **설치할 것이 없습니다** — `package.json`도
`node_modules`도 만들지 않았습니다.
...
**대시보드는 API와 같은 프로세스가 내보냅니다.** 정적 파일 서버를 따로 띄우면
오리진이 갈려 CORS 설정이 필요해지고, `file://`로 열면 브라우저가 같은 폴더의
JSON `fetch`를 막습니다.
...
<!-- 스크린샷/GIF는 M5(문서 마무리)에서 추가합니다. -->

서버를 띄운 뒤 아래 주소로 화면 상태를 하나씩 확인할 수 있습니다. 페이지 상단의
전환기로도 오갈 수 있습니다.

| 주소 | 화면 |
|---|---|
| `/?repo=PrefectHQ/fastmcp` | 실시간 API |
| `/?fixture=ready` | 지표가 채워진 상태 |
| `/?fixture=pending` | **수집 전** — 점선 카드, 숫자 없음 |
| `/?fixture=empty` | **분모 0** — 실선 카드, 비율만 없고 건수는 그대로 |
| `/?repo=nobody/nothing` | 이 대시보드가 모르는 저장소 (404) |

`?fixture=`는 API를 호출하지 않고 `frontend/fixtures/`의 고정 응답을 읽습니다.

**뒤의 두 상태를 갈라 둔 것이 이 화면의 핵심입니다.** 응답의 `null`은 자리에 따라
뜻이 다릅니다 — `status="pending"`일 때의 지표는 "아직 계산할 수 없다"(기다리면
값이 생긴다)이고, `ratio`·`median_seconds`의 `null`은 "비율을 낼 수 없다"(분모가
0이라 기다려도 안 생긴다)입니다. 둘 다 회색 대시로 그리면 사용자는 기다려야 할지
알 수 없고, **0으로 채우면 빈 저장소가 가장 건강해 보입니다.**

세지 못한 건수(미분석 · 메인테이너 응답 없음 · 첫 응답 미조사)도 화면에서 숨기지
않습니다. 분모에서 조용히 빠지면 비율은 늘 그럴듯해 보이고, 무엇이 빠졌는지는
어디에도 남지 않습니다.
...
| M4 | API · 대시보드 | 완료 |
| M5 | 문서 마무리 (README 일본어 · 스크린샷 · 설계 요약) | 예정 |
